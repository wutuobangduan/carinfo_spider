...
def get_car_info():
    imgurls = [""]
    try:
        conn = MySQLdb.connect(host='21',user='',passwd='',charset='utf8')
        curs = conn.cursor()
        conn.select_db('')
...
def post_cardata():
    ...
    if browser is not None:  
        while True:
            try:
                if get_page_fails > 10:
                    break
                browser.get("http://passport.58.com/login?path=http%3A//post.58.com/172/29/s5%3Fpts%3D1426815421662&PGTID=14268154223350.3146644136868417&ClickID=1")
                browser.implicitly_wait(5)
            except:
                get_page_fails += 1
        ...
        browser.find_element_by_id('username').send_keys(options.username)
        ...
        printDelimiter()
        print u"品牌:",brand
        print u"车系:",vehicle_series
        print u"车型:",vehicle_model
        ...
        browser.find_element_by_name('p_baoyang').click()
        browser.find_element_by_name('p_shiguqk').click()
        browser.switch_to.default_content()
        time.sleep(2)
        ...
        browser.find_element_by_id('MinPrice').send_keys(shown_miles.decode('utf-8'))
        ...
        for i in range(0,2):
            print car_imgs[i]
            browser.find_element_by_id('PictureUpload').click()            
