...
#            reference_prices = reference_price.text
        ...
        for div in soup.find_all('div',attrs={'class':'time'}):
            if len(str(div.get_text()).split('\n'))>1:
                release_time = current_date + ' ' + str(div.get_text()).split('\n')[1].replace(' ','')
        #print release_time
        if telephone != '':
            ...
            try:
                ...
                curs.execute("select id from che168_sell_car_info where url='%s'" % myUrl)
                getrows=curs.fetchall()
                is_seller = u'个人'.encode('utf-8')
                if not getrows:
                    curs.execute("select id from che168_sell_car_info where telephone='%s'" % telephone)
                    get_telephones = curs.fetchall()
                    if not get_telephones:
                        is_seller = u'个人'.encode('utf-8')
                    else:
                        is_seller = u'商家'.encode('utf-8')
                    res = [telephone,is_seller,name,addrs,title,prices,reference_prices,release_time,trip_distance,licenses,owner_readme,myUrl]
                    print "The data is not exists in the database..."
                    curs.execute("insert into che168_sell_car_info(telephone,is_seller,name,addrs,title,prices,reference_prices,release_time,trip_distance,licenses,owner_readme,url) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",res)
                else:
...
def main():
    url="http://www.che168.com/jiangsu/a0_0ms1dgscncgpiltocsp"
    localfile="Href.txt"
    for i in range(1,50):
        myUrl = url + str(i)+'ex/'
