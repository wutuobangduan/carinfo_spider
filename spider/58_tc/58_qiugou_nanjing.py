...
#        print td.get_text()
    for div in soup.find_all('div',attrs={"class":"infolist"}):
        print div.get_text()
        for talbe in div.find_all('talbe',attrs={"class":"tblist"}):
            for td in table.find_all('td',attrs={"class":"t"}):
                print td.get_text()
                for a in td.find_all('a',attrs={"class":"t"}):
                    print a.get('href')
...
def get_qiugou_info(myUrl):
    ...
    #    urllib.urlretrieve(img_src,'/data/python/spider/58_tc/nanjing/%s.jpg' % uni_number)
    ...
    if img_src.find('ttp')>0:
        #try:
        #    fp = open("/data/python/spider/58_tc/nanjing/%s.jpg" % uni_number)
        #    myImg = fp.read()
        #    fp.close()
        #except IOError,e:
...
def main():
    url="http://nj.58.com/ershoucheqg/pn"
    localfile="Href.txt"
    for i in range(1,5):
        myUrl = url + str(i)
