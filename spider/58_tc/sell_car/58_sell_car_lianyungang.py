    ...
    for content in BeautifulSoup(html).find_all('td',attrs={'class':'t'}):
        for a in content.find_all('a',attrs={'class':'t'}):
                ...
                if len(a.get('href')).split('/'))>2 and len(str(a.get('href')).split('/')[2].split('.'))>0:
                    if str(a.get('href')).split('/')[2].split('.')[0] == 'lyg':
...
def get_qiugou_info(myUrl):
    #proxy = {'http':'http://60.28.196.69:80'}
    ...
    if html != '':
        ...
        if telephone != '':
            ...
            try:
                ...
                if not getrows:
                    curs.execute("select id from 58_sell_car_info where telephone='%s'" % telephone)
...
#get_qiugou_info('http://lyg.58.com/ershouche/20647753356038x.shtml?PGTID=14213939221850.6898022816450731&ClickID=4')
def main():
    url="http://lyg.58.com/ershouche/0/pn"
    localfile="Href.txt"
    for i in range(1,30):
        myUrl = url + str(i)
