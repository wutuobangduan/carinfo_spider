...
def get_qiugou_info(myUrl):
    ...
    if myUrl.split('/')[2].split('.')[0] == 'hf':
        addr = u'安徽合肥'.encode('utf-8')
    elif myUrl.split('/')[2].split('.')[0] == 'wuhu':
        addr = u'安徽芜湖'.encode('utf-8')
    elif myUrl.split('/')[2].split('.')[0] == 'bengbu':
        addr = u'安徽蚌埠'.encode('utf-8')
    elif myUrl.split('/')[2].split('.')[0] == 'fy':
        addr = u'安徽阜阳'.encode('utf-8')
    elif myUrl.split('/')[2].split('.')[0] == 'hn':
        addr = u'安徽淮南'.encode('utf-8')
    elif myUrl.split('/')[2].split('.')[0] == 'anqing':
        addr = u'安徽安庆'.encode('utf-8')
    elif myUrl.split('/')[2].split('.')[0] == 'suzhou':
        addr = u'安徽宿州'.encode('utf-8')
    elif myUrl.split('/')[2].split('.')[0] == 'la':
        addr = u'安徽六安'.encode('utf-8')
    elif myUrl.split('/')[2].split('.')[0] == 'huaibei':
        addr = u'安徽淮北'.encode('utf-8')
    elif myUrl.split('/')[2].split('.')[0] == 'chuzhou':
        addr = u'安徽滁州'.encode('utf-8')
    elif myUrl.split('/')[2].split('.')[0] == 'mas':
        addr = u'安徽马鞍山'.encode('utf-8')
    elif myUrl.split('/')[2].split('.')[0] == 'tongling':
        addr = u'安徽铜陵'.encode('utf-8')
    elif myUrl.split('/')[2].split('.')[0] == 'xuancheng':
        addr = u'安徽宣城'.encode('utf-8')
    elif myUrl.split('/')[2].split('.')[0] == 'bozhou':
        addr = u'安徽亳州'.encode('utf-8')
    elif myUrl.split('/')[2].split('.')[0] == 'huangshan':
        addr = u'安徽黄山'.encode('utf-8')
    elif myUrl.split('/')[2].split('.')[0] == 'chizhou':
        addr = u'安徽池州'.encode('utf-8')
    elif myUrl.split('/')[2].split('.')[0] == 'ch':
        addr = u'安徽巢湖'.encode('utf-8')
    elif myUrl.split('/')[2].split('.')[0] == 'hexian':
        addr = u'安徽和县'.encode('utf-8')
    elif myUrl.split('/')[2].split('.')[0] == 'hq':
        addr = u'安徽霍邱'.encode('utf-8')
    elif myUrl.split('/')[2].split('.')[0] == 'tongcheng':
        addr = u'安徽桐城'.encode('utf-8')
    else:
        addr = None
    #if img_src.find('ttp')>0:
        #try:
...
def main():
    addr_list = ['hf','wuhu','bengbu','fy','hn','anqing','suzhou','la','huaibei','chuzhou','mas','tongling','xuancheng','bozhou','huangshan','chizhou','ch','hexian','hq','tongcheng']
