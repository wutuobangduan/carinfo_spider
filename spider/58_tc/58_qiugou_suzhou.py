...
def grabHref(url,localfile):
    ...
    pat = re.compile(r'http://su.58.com/ershoucheqg/\d+x.shtml')
...
def get_qiugou_info(myUrl):
    ...
    #    urllib.urlretrieve(img_src,'/data/python/spider/58_tc/suzhou/%s.jpg' % uni_number)
    ...
    if img_src.find('ttp')>0:
        #try:
        #    fp = open("/data/python/spider/58_tc/suzhou/%s.jpg" % uni_number)
        #    myImg = fp.read()
        #    fp.close()
        #except IOError,e:
        #    print "Read image,Error %d %s" % (e.args[0],e.args[1])
        #    sys.exit(1)
    ...
    try:
        ...
        print "Execute sql,Error %d %s" % (e.args[0],e.args[1])
        sys.exit(1)
...
def main():
    url="http://su.58.com/ershoucheqg/pn"
    localfile="Href.txt"
    for i in range(1,5):
        myUrl = url + str(i)
