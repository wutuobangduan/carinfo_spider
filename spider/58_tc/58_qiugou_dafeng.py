...
def grabHref(url,localfile):
    ...
    pat = re.compile(r'http://dafeng.58.com/ershoucheqg/\d+x.shtml')
...
def get_qiugou_info(myUrl):
    ...
    #    urllib.urlretrieve(img_src,'/data/python/spider/58_tc/dafeng/%s.jpg' % uni_number)
    ...
    if img_src.find('ttp')>0:
        #try:
        #    fp = open("/data/python/spider/58_tc/dafeng/%s.jpg" % uni_number)
        #    myImg = fp.read()
        #    fp.close()
        #except IOError,e:
...
def main():
    url="http://dafeng.58.com/ershoucheqg/pn"
    localfile="Href.txt"
    for i in range(1,5):
        myUrl = url + str(i)
