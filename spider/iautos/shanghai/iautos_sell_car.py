...
def get_qiugou_info(myUrl):
    ...
    if html != '':
        ...
        if u'上海'.encode('utf-8') not in addrs:
            addrs = u'上海'.encode('utf-8') + addrs     
        if telephone != '':
            ...
            try:
                ...
                if not getrows:
                    ...
                    if not get_telephones:
                        ...
                        is_seller = u'商家'.encode('utf-8')
                    if telephone.rstrip(',').startswith("400"):
                        is_seller = u'商家'.encode('utf-8')
...
def main():
    url="http://so.iautos.cn/shanghai/p"
    localfile="Href.txt"
    for i in range(1,5):
        print "current page is %d" % i
        myUrl = url + str(i)+'as2dsvepcatcpbnscac/'
