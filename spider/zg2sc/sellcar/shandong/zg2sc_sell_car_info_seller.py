    ...
    if html != '':
        ...
        if len(title.split('['))>1:
            if len(title.split('[')[1].split(']'))>0:
                if u'济南'.encode('utf-8') in title.split('[')[1].split(']')[0] or u'青岛'.encode('utf-8') in title.split('[')[1].split(']')[0] or u'淄博'.encode('utf-8') in title.split('[')[1].split(']')[0] or u'枣庄'.encode('utf-8') in title.split('[')[1].split(']')[0] or u'东营'.encode('utf-8') in title.split('[')[1].split(']')[0] or u'烟台'.encode('utf-8') in title.split('[')[1].split(']')[0] or u'潍坊'.encode('utf-8') in title.split('[')[1].split(']')[0] or u'济宁'.encode('utf-8') in title.split('[')[1].split(']')[0] or u'泰安'.encode('utf-8') in title.split('[')[1].split(']')[0] or u'威海'.encode('utf-8') in title.split('[')[1].split(']')[0] or u'日照'.encode('utf-8') in title.split('[')[1].split(']')[0] or u'莱芜'.encode('utf-8') in title.split('[')[1].split(']')[0] or u'临沂'.encode('utf-8') in title.split('[')[1].split(']')[0] or u'德州'.encode('utf-8') in title.split('[')[1].split(']')[0] or u'聊城'.encode('utf-8') in title.split('[')[1].split(']')[0] or u'滨州'.encode('utf-8') in title.split('[')[1].split(']')[0] or u'菏泽'.encode('utf-8') in title.split('[')[1].split(']')[0]:
                    addrs = u"山东".encode('utf-8') + title.split('[')[1].split(']')[0]
...
def main():
    ...
    for i in range(1,5):
        ...
        myUrl = url + str(i) + "&sr=null&prov=%C9%BD%B6%AB"
