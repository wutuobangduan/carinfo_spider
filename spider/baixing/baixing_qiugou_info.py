    ...
    if html!='':
        ...
        if telephone != '':
            try:
                ...
                curs.execute("select id from baixing_sell_car_info where url='%s'" % myUrl)
                getrows=curs.fetchall()
                if not getrows:
                    curs.execute("select id from baixing_sell_car_info where telephone='%s'" % telephone)
                    get_telephones = curs.fetchall()
                    if not get_telephones:
                        is_seller = u'个人'.encode('utf-8')
                    else:
                        is_seller = u'商家'.encode('utf-8')
                    res= [title.next,is_seller,telephone,prices,release_time,brands,vehicle_series,vehicle_models,registration_dates,trip_distances,addrs,displacements,vehicle_colors,transmissions,major_accidents,model_levels,detail_informations,myUrl]
                    print "The data is not exists in the database..."
                    curs.execute("insert into baixing_sell_car_info(title,is_seller,telephone,prices,release_time,brands,vehicle_series,vehicle_models,registration_dates,trip_distances,addrs,displacements,vehicle_colors,transmissions,major_accidents,model_levels,detail_informations,url) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",res)
                else:
...
def main():
    url="http://jiangsu.baixing.com/ershouqiche/?page="
    localfile="Href.txt"
    for i in range(1,50):
        myUrl = url + str(i)
