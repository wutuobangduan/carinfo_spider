...
def get_qiugou_info(myUrl):
    ...
    if html != '':
        ...
        if myUrl.split('/')[2].split('.')[0] == 'jn':
            addrs = u'山东济南'.encode('utf-8')
        elif myUrl.split('/')[2].split('.')[0] == 'qd':
            addrs = u'山东青岛'.encode('utf-8')
        elif myUrl.split('/')[2].split('.')[0] == 'wei':
            addrs = u'山东威海'.encode('utf-8')
        elif myUrl.split('/')[2].split('.')[0] == 'zibo':
            addrs = u'山东淄博'.encode('utf-8')
        elif myUrl.split('/')[2].split('.')[0] == 'zaozhuang':
            addrs = u'山东枣庄'.encode('utf-8')
        elif myUrl.split('/')[2].split('.')[0] == 'dongying':
            addrs = u'山东东营'.encode('utf-8')
        elif myUrl.split('/')[2].split('.')[0] == 'yantai':
            addrs = u'山东烟台'.encode('utf-8')
        elif myUrl.split('/')[2].split('.')[0] == 'weifang':
            addrs = u'山东潍坊'.encode('utf-8')
        elif myUrl.split('/')[2].split('.')[0] == 'laiwu':
            addrs = u'山东莱芜'.encode('utf-8')
        elif myUrl.split('/')[2].split('.')[0] == 'binzhou':
            addrs = u'山东滨州'.encode('utf-8')
        elif myUrl.split('/')[2].split('.')[0] == 'jining':
            addrs = u'山东济宁'.encode('utf-8')
        elif myUrl.split('/')[2].split('.')[0] == 'taian':
            addrs = u'山东泰安'.encode('utf-8')
        elif myUrl.split('/')[2].split('.')[0] == 'rizhao':
            addrs = u'山东日照'.encode('utf-8')
        elif myUrl.split('/')[2].split('.')[0] == 'linyi':
            addrs = u'山东临沂'.encode('utf-8')
        elif myUrl.split('/')[2].split('.')[0] == 'dezhou':
            addrs = u'山东德州'.encode('utf-8')
        elif myUrl.split('/')[2].split('.')[0] == 'liaocheng':
            addrs = u'山东聊城'.encode('utf-8')
        elif myUrl.split('/')[2].split('.')[0] == 'heze':
            addrs = u'山东菏泽'.encode('utf-8')
        else:
            addrs = None
        name = ''
        brand_model = ''
...
def main():
    addr_list = ['jn','qd','wei','zibo','zaozhuang','dongying','yantai','weifang','laiwu','binzhou','jining','taian','rizhao','linyi','dezhou','liaocheng','heze']
    for j in range(len(addr_list)):
        url = "http://" + addr_list[j] + ".ganji.com/ershouche/a2o"
        localfile="Href.txt"
        for i in range(1,4):
