...
def get_qiugou_info(myUrl):
    ...
    if html != '':
        ...
        if myUrl.split('/')[2].split('.')[0] == 'hz':
            addrs = u'浙江杭州'.encode('utf-8')
        elif myUrl.split('/')[2].split('.')[0] == 'nb':
            addrs = u'浙江宁波'.encode('utf-8')
        elif myUrl.split('/')[2].split('.')[0] == 'wenzhou':
            addrs = u'浙江温州'.encode('utf-8')
        elif myUrl.split('/')[2].split('.')[0] == 'jiaxing':
            addrs = u'浙江嘉兴'.encode('utf-8')
        elif myUrl.split('/')[2].split('.')[0] == 'huzhou':
            addrs = u'浙江湖州'.encode('utf-8')
        elif myUrl.split('/')[2].split('.')[0] == 'shaoxing':
            addrs = u'浙江绍兴'.encode('utf-8')
        elif myUrl.split('/')[2].split('.')[0] == 'jinhua':
            addrs = u'浙江金华'.encode('utf-8')
        elif myUrl.split('/')[2].split('.')[0] == 'quzhou':
            addrs = u'浙江衢州'.encode('utf-8')
        elif myUrl.split('/')[2].split('.')[0] == 'zhoushan':
            addrs = u'浙江舟山'.encode('utf-8')
        elif myUrl.split('/')[2].split('.')[0] == 'zjtaizhou':
            addrs = u'浙江台州'.encode('utf-8')
        elif myUrl.split('/')[2].split('.')[0] == 'lishui':
            addrs = u'浙江丽水'.encode('utf-8')
        else:
            addrs = None
        name = ''
        brand_model = ''
...
def main():
    addr_list = ['hz','nb','wenzhou','jiaxing','huzhou','shaoxing','jinhua','quzhou','zhoushan','zjtaizhou','lishui']
