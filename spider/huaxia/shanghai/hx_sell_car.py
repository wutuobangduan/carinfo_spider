            ...
            for div2 in div.find_all('div',attrs={'class':'infor_r'}):
                ...
                for p in div2.find_all('p',attrs={'class':'carplace'}):
                    for span in p.find_all('span',attrs={'class':'context_name'}):
                        ...
                        for span in p.find_all('span'):
                            for b in span.find_all('b'):
                                if u'上海'.encode('utf-8') in str(b.get_text()):
        ...
        if telephone != '':
            ...
            try:
                ...
                if not getrows:
                    ...
                    info_src = 'huaxia'
                    res = [title,name,telephone,addrs,release_time,prices,is_seller,owner_readme,info_src,myUrl]
                    print "The data is not exists in the database..."
                    curs.execute("insert into sell_car_info(title,name,telephone_num,addrs,release_time,prices,is_seller,owner_readme,info_src,url) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",res)
                else:
                    print 'The data is already in the database...'
                    curs.execute("update sell_car_info set prices ='%s' where url ='%s'" % (prices,myUrl))
                conn.commit()
                curs.close()
                conn.close()
...
def main():
    url="http://www.hx2car.com/car/search.htm?carFlag=personal&more=l310000jyckbmg&currPage="
    localfile="Href.txt"
    for i in range(1,5):
