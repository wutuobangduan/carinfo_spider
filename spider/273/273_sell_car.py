...
def get_qiugou_info(myUrl):
    ...
    if html != '':
        ...
        if telephone != '':
            print title.strip(),release_time[0],addrs,name,telephone.rstrip(tele),prices,trip_distance,licenses,emission_standard,owner_readme,myUrl
            res = [title.strip(),release_time[0],addrs,name,telephone.rstrip(tele),prices,trip_distance,licenses,emission_standard,owner_readme,myUrl]
            try:
                conn = MySQLdb.connect(host='127.0.0.1',user='root',passwd='dp')
                curs = conn.cursor()
                conn.select_db('spider')
                curs.execute("select id from 273_sell_car_info where url='%s'" % myUrl)
                getrows=curs.fetchall()
                if not getrows:
                    print "The data is not exists in the database..."
                    curs.execute("insert into 273_sell_car_info(title,release_time,addrs,name,telephone,prices,trip_distance,licenses,emission_standard,owner_readme,url) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",res)
                else:
...
#get_qiugou_info('http://nj.273.cn/car/15139713.html')

#print "=================================================================================="

#get_qiugou_info('http://su.273.cn/car/14190218.html')

#print "=================================================================================="

#get_qiugou_info('http://lyg.273.cn/car/15028002.html')
