    ...
    if html != '':
        ...
        for div in soup.find_all('div',attrs={'class':'car-act clearfix'}):
            for div2 in soup.find_all('div',attrs={'class':'push-time'}):      
                release_time = current_date + ',' + str(div2.get_text())
        if telephone != '':
            ...
            try:
                ...
                curs.execute("select id from dasouche_sell_car_info where url='%s'" % myUrl)
                getrows=curs.fetchall()
                if not getrows:
                    curs.execute("select id from dasouche_sell_car_info where telephone='%s'" % telephone)
                    get_telephones = curs.fetchall()
                    if not get_telephones:
                        is_seller = u'个人'.encode('utf-8')
                    else；
                        is_seller = u'商家'.encode('utf-8')
                    res = [title,is_seller,name,release_time,prices,new_car_prices,telephone,addrs,licenses,trip_distance,emission_standard,myUrl]
                    print "The data is not exists in the database..."
                    curs.execute("insert into dasouche_sell_car_info(title,is_seller,name,release_time,prices,new_car_prices,telephone,addrs,licenses,trip_distance,emission_standard,url) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",res)
                else:
