...
def update_vehicle_model():
    try:
        ...
        for i in xrange(15000,17000):
            conn = MySQLdb.connect(host='127.0.0.1',user='root',passwd='dp')
            curs = conn.cursor()
            conn.select_db('tc_platform')
            curs.execute("select che168_model_id from tc_vehicle_model where vehicle_model_id=%s" % i)
            ...
            curs.execute("update tc_vehicle_model set spec_value = %s where vehicle_model_id=%s",(config_detail.encode('utf-8'),i))
            conn.commit()
            curs.close()
            conn.close()
