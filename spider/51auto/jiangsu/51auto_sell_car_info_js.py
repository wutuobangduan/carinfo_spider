...
def get_qiugou_info(myUrl):
    ...
    if html != '':
            ...
            for i in p.find_all('i'):
                for span in p.find_all('span'):
                    ...
                    if addr in str(span.get_text()):
                        if u'江苏'.encode('utf-8') in str(i.get_text()):
        ...
        if img_src != '':
            ...
            #res = [title,telephone_num,name,addrs,release_time,prices,licenses,trip_distances,displacements,transmissions,vehicle_colors,effluent_standard,img_src,owner_readme,myUrl]
            ...
            try:
                ...
                if not getrows:
                    curs.execute("select id from sell_car_info where img_src ='%s'" % img_src)
